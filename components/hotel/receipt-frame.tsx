...
export function ReceiptFrame({ children }: { children: ReactNode }) {
  ...
  const btn = (v: "58" | "80") =>
    `rounded-lg border px-3 py-1.5 text-xs font-medium ${
      w === v ? "border-amber-400 bg-amber-50 text-amber-800" : "border-slate-300 text-slate-600"
    }`;

  return (
    <div>
      <div className="no-print mb-3 flex flex-wrap items-center gap-2">
        <span className="text-xs text-slate-500">Receipt width:</span>
      ...
      <div
        className="mx-auto rounded-2xl border border-slate-200 bg-white p-4 text-xs leading-tight text-slate-900 print:rounded-none print:border-0 print:p-1"

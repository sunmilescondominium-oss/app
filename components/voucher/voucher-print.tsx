...
function VoucherBody({ data }: { data: VoucherData }) {
  const {
    rcNo, date, place = "Calamba City", paidTo, paidToAddress = "",
    particulars, totalAmount, voucherNo, sourceOfFunds, checkNumber, isReprint,
  } = data;
    ...
    <div style={{ position: "relative" }}>
      {isReprint && (
        <div style={{
          ...
          <span style={{
            fontSize: "52pt", fontWeight: "bold", color: "rgba(200,0,0,0.12)",
            transform: "rotate(-25deg)", letterSpacing: "4px", textTransform: "uppercase",
            userSelect: "none",
          }}>REPRINT</span>
        </div>
      )}
      <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed", position: "relative", zIndex: 2 }}>
        <tbody>
          {/* Header */}
          <tr>
          ...
          {/* Voucher No + VOUCHER title + RC No */}
          <tr>
            <td style={{ ...labelCol }}>Voucher No.</td>
            <td style={{ ...td, width: "22%", fontWeight: "bold" }}>{voucherNo ?? "—"}</td>
            <td colSpan={3} style={{ ...td, textAlign: "center", fontSize: "14pt", fontWeight: "bold", letterSpacing: "2px" }}>
              VOUCHER
              {isReprint && (
                <span style={{ fontSize: "8pt", fontWeight: "normal", marginLeft: "8px", color: "#c00", verticalAlign: "middle" }}>
                  [REPRINT]
                </span>
              )}
            </td>
            <td style={{ ...td, width: "14%" }}>
              <span style={{ fontSize: "8pt", color: "#666" }}>R.C No.</span>
              <div style={{ fontWeight: "bold", fontSize: "9pt" }}>{rcNo}</div>
            </td>
          </tr>

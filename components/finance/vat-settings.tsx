...
export function VatSettings({ settings }: { settings: FinanceSettings }) {
  ...
  return (
    <div className="no-print flex flex-wrap items-end gap-2 rounded-2xl border border-stone-200 bg-white p-4">
      <div>
        <label className="mb-1 block text-xs font-medium text-stone-600">Sales VAT</label>
        <select
          value={mode}
          onChange={(e) => {
      ...
      <span className="text-xs text-stone-400">Confirm the regime with accounting.</span>
    </div>
  );
}

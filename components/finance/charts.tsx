...
export function SalesByLineChart({ rows }: { rows: { label: string; gross: number }[] }) {
  ...
  if (rows.length === 0) return <p className="text-sm text-slate-400">No sales to chart.</p>;
  return (
    <div className="space-y-2.5">
      {rows.map((r) => (
        <div key={r.label} className="flex items-center gap-3 text-sm">
          <span className="w-24 shrink-0 truncate text-slate-600" title={r.label}>{r.label}</span>
          <div className="h-5 flex-1 overflow-hidden rounded bg-slate-100">
            <div className="h-full rounded bg-amber-500" style={{ width: `${(r.gross / max) * 100}%` }} />
          </div>
          <span className="w-24 shrink-0 text-right tabular-nums text-slate-700">{peso(r.gross)}</span>
        </div>
      ))}
    </div>
  );
...
export function MonthlyChart({ points }: { points: MonthPoint[] }) {
  ...
  if (points.length === 0) return <p className="text-sm text-slate-400">No monthly data.</p>;
  return (
    <div>
      <div className="flex h-40 items-end gap-3">
        {points.map((p) => (
          <div key={p.month} className="flex flex-1 flex-col items-center gap-1">
            ...
            <span className="text-[11px] text-slate-500">{p.month.slice(5)}</span>
          </div>
        ))}
      </div>

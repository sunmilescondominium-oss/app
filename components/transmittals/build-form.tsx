  ...
  return (
    ...
    >
      ...
      <div>
        ...
        {loadState === "loading" && (
          <p className="py-3 text-sm text-stone-400">Loading collections…</p>
        )}
        {loadState === "error" && (
          <p className="py-2 text-sm text-red-600">{loadErr}</p>
        )}
        {loadState === "empty" && (
      ...
      {paymentMode === "cash" && <DenominationCounter />}

      {/* Bank transfer: bank account + proof */}
      {paymentMode === "bank_transfer" && (
        <div className="space-y-4 rounded-xl border border-stone-200 bg-stone-50 p-4">
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-stone-600">
              Sun Miles bank account deposited to
            </label>
            ...
            {bankAccounts.length === 0 && (
              <p className="mt-1 text-xs text-amber-700">
                No bank accounts configured yet — add them in the Banking module first.
              </p>
            )}
          </div>
          <div>
      ...
      <div className="flex items-center gap-3">
        <button
          type="submit"
          disabled={pending || selectedIds.size === 0 || loadState === "loading"}
        ...
        {!pending && loadState === "done" && selectedIds.size > 0 && (
          <span className="text-xs text-stone-500">
            Will bundle {selectedIds.size} collection(s) · {peso(selectedTotal)}
          </span>
        )}
      </div>
    </form>

    ...
    <div className="flex flex-wrap items-end gap-3 rounded-2xl border border-stone-200 bg-white p-4">
      ...
      {canWrite && status === "submitted" && (
        <form action={depAction} className="flex flex-wrap items-end gap-2">
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-stone-600">Amount deposited (₱)</label>
            <input
              name="deposited_amount"
              type="number"
              step="0.01"
              min="0"
              className="w-32 rounded-lg border border-stone-300 px-3 py-2 text-sm outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-200"
            />
          </div>
          <button
            type="submit"
      ...
      {canReconcile && (status === "deposited" || status === "reconciled") && !passbookReturned && (
        <button
          ...
          onClick={passbook}
          disabled={busy}
          className="rounded-lg border border-stone-300 px-4 py-2 text-sm font-semibold text-stone-700 hover:bg-stone-100 disabled:opacity-60"
        >

...
function Row({ s }: { s: Series }) {
  ...
  return (
    <div className="flex flex-wrap items-end gap-2">
      <span className="w-16 pb-2 text-sm font-medium capitalize text-stone-600">{s.context}</span>
      <div>
        <label className="mb-1 block text-[11px] text-stone-500">Prefix</label>
        <input value={prefix} onChange={(e) => setPrefix(e.target.value)} className={`${cls} w-24`} />
      </div>
      <div>
        <label className="mb-1 block text-[11px] text-stone-500">Next number</label>
        <input type="number" min="1" value={nextNo} onChange={(e) => setNextNo(Number(e.target.value))} className={`${cls} w-28`} />
      </div>
      <span className="pb-2 text-xs text-stone-400">next: {prefix}{String(nextNo).padStart(6, "0")}</span>
...
export function ReceiptSeriesPanel({ series }: { series: Series[] }) {
  return (
    <div className="no-print mb-6 rounded-2xl border border-stone-200 bg-white p-4">
      <p className="mb-1 text-sm font-semibold text-stone-800">Acknowledgement Receipt series</p>
      <p className="mb-3 text-xs text-stone-400">Set the series to match your physical AR booklets (hotel & rental). New receipts continue from the next number.</p>

...
export function CleaningPhotos({ taskId, count, canWrite }: { taskId: string; count: number; canWrite: boolean }) {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4">
      <p className="mb-2 text-sm font-semibold text-slate-700">Cleaning photos</p>
        ...
        <div className="mb-3 flex flex-wrap gap-2">
          {Array.from({ length: count }).map((_, i) => (
            <a key={i} href={`/api/housekeeping/${taskId}/photo?i=${i}`} target="_blank" rel="noreferrer" className="block">
              ...
              <img src={`/api/housekeeping/${taskId}/photo?i=${i}`} alt={`Photo ${i + 1}`} className="h-20 w-20 rounded-lg object-cover ring-1 ring-slate-200" />
            </a>
          ))}
        </div>
      ) : (
        <p className="mb-3 text-xs text-slate-400">No photos yet — take pictures of the bed, toilet, and room after cleaning.</p>
      )}

      {canWrite && (
        <div className="flex flex-wrap items-center gap-2">
          <select value={area} onChange={(e) => setArea(e.target.value)} className="rounded-lg border border-slate-300 px-2 py-1.5 text-sm">

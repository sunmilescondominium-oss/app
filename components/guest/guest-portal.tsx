  ...
  return (
    <div>
      ...
      <div className={`rounded-xl p-4 text-center ${overtime ? "bg-rose-50" : "bg-slate-50"}`}>
        <p className="text-xs uppercase tracking-wide text-slate-500">{overtime ? "Time over by" : "Time remaining"}</p>
        <p className={`mt-1 text-3xl font-bold tabular-nums ${overtime ? "text-rose-700" : "text-slate-900"}`}>{fmt(remaining)}</p>
        <p className="mt-1 text-xs text-slate-400">Planned checkout: {new Date(outMs).toLocaleString("en-PH", { timeZone: "Asia/Manila", hour: "2-digit", minute: "2-digit", month: "short", day: "numeric" })}</p>
      ...
      <table className="mt-4 w-full text-left text-sm">
        <tbody>
          <tr className="border-b border-slate-100"><td className="py-1.5">Room charge (net of discount)</td><td className="py-1.5 text-right tabular-nums">{peso(stay.total - stay.ordersTotal)}</td></tr>
          {stay.orders.map((o, i) => (
            <tr key={i} className="border-b border-slate-100"><td className="py-1.5 text-slate-600">{o.name} ×{o.qty}</td><td className="py-1.5 text-right tabular-nums">{peso(o.amount)}</td></tr>
          ))}
          <tr className="border-b border-slate-100 font-semibold"><td className="py-2">Total</td><td className="py-2 text-right tabular-nums">{peso(stay.total)}</td></tr>
          <tr className="border-b border-slate-100"><td className="py-1.5">Paid</td><td className="py-1.5 text-right tabular-nums">{peso(stay.paid)}</td></tr>
      ...
      {active ? (
        <div className="mt-5 space-y-3">
          <div className="flex items-end gap-2">
            <div>
              <label className="mb-1 block text-xs font-medium text-slate-600">Request more time</label>
              <select value={hours} onChange={(e) => setHours(Number(e.target.value))} className="rounded-lg border border-slate-300 px-2.5 py-2 text-sm">
          ...
          {stay.checkoutRequested && <p className="text-center text-xs text-slate-500">The front desk will finalize your bill and issue the gate pass after the room check.</p>}
        </div>
      ) : (
        <p className="mt-5 rounded-lg bg-slate-50 px-3 py-2 text-center text-sm text-slate-500">This stay is checked out.</p>
      )}

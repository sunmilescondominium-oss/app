...
const cls = "w-full rounded-lg border border-stone-300 px-2.5 py-2 text-sm outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-200";
...
export function RenterDetails({ lease }: { lease: Lease }) {
  ...
  return (
    <form action={action} className="rounded-2xl border border-stone-200 bg-white p-5">
      ...
      <h3 className="mb-3 text-sm font-semibold text-stone-700">Renter details</h3>
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <label className="block">
          <span className="mb-1 block text-xs font-medium text-stone-600">Lease type</span>
      ...
      <p className="mt-1 text-[11px] text-stone-400">Give the tenant their unit number + PIN to view bills at /renter-portal.</p>
...
function DocRow({ leaseId, doc }: { leaseId: string; doc: LeaseDoc }) {
  ...
  return (
    <form action={submit} className="flex flex-wrap items-center gap-2 border-b border-stone-100 py-2 last:border-0 text-sm">
      <span className={`h-2.5 w-2.5 shrink-0 rounded-full ${doc.submitted ? "bg-emerald-500" : "bg-stone-300"}`} />
      <span className="min-w-[10rem] flex-1 font-medium text-stone-700">{doc.docType}</span>
      ...
      <button type="submit" disabled={busy} className="rounded-lg border border-stone-300 px-2.5 py-1 text-xs font-medium text-stone-700 hover:bg-stone-50 disabled:opacity-50">
        {busy ? "…" : "Save"}
      </button>
    </form>
  );
  ...
  return (
    <div className="rounded-2xl border border-stone-200 bg-white p-4">
      <div className="mb-2 flex items-center justify-between">
        <h3 className="text-sm font-semibold text-stone-700">Document checklist</h3>
        <span className="text-xs text-stone-400">{done}/{LEASE_DOC_TYPES.length} submitted</span>
      </div>

...
export function KioskSettingsPanel({ accessCode, showPhotos }: { accessCode: string; showPhotos: boolean }) {
  ...
  return (
    <div className="no-print flex flex-wrap items-end gap-3 rounded-2xl border border-slate-200 bg-white p-4">
      <div>
        <label className="mb-1 block text-xs font-medium text-slate-600">Kiosk access code</label>
        <input
          value={code}
          onChange={(e) => setCode(e.target.value)}
          placeholder="blank = open"
          className="w-40 rounded-lg border border-slate-300 px-2.5 py-2 text-sm outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-200"
        />
      </div>

...
export function LeaveForm() {
  ...
  return (
    <form action={action} className="flex flex-wrap items-end gap-2 rounded-2xl border border-stone-200 bg-white p-4">
      ...
      <p className="w-full text-xs text-stone-400">Please file at least {LEAVE_MIN_LEAD_DAYS} days ahead so coverage can be arranged.</p>
...
export function ObForm() {
  ...
  return (
    <form action={action} className="flex flex-wrap items-end gap-2 rounded-2xl border border-sky-200 bg-sky-50/40 p-4">
      <div>
        <label className="mb-1 block text-xs font-medium text-stone-600">OB date</label>
        <input type="date" name="start_date" required className={cls} />
      </div>
      <div>
        <label className="mb-1 block text-xs font-medium text-stone-600">Until (optional)</label>
      ...
      <div className="min-w-[10rem] flex-1">
        <label className="mb-1 block text-xs font-medium text-stone-600">Where / purpose</label>
      ...
      <p className="w-full text-xs text-stone-400">Official Business needs approval. It is auto-cancelled if you clock in on that day.</p>
      {state && !state.ok && <p className="w-full text-sm text-red-700">{state.error}</p>}
    </form>
  );
}
...
export function RequestForm() {
  ...
  return (
    <form action={action} className="flex flex-wrap items-end gap-2 rounded-2xl border border-stone-200 bg-white p-4">
      <div>
        <label className="mb-1 block text-xs font-medium text-stone-600">Request</label>
      ...
      {needsHours && (
        <div>
          <label className="mb-1 block text-xs font-medium text-stone-600">Hours</label>
          <input type="number" name="hours" step="0.5" min="0" max="24" className={`${cls} w-20`} />
        </div>
      )}
      {category === "other" && (
        <div>
          <label className="mb-1 block text-xs font-medium text-stone-600">Subject</label>
          <input name="subject" placeholder="e.g. Certificate" className={cls} />
        </div>
      )}
      <div className="min-w-[10rem] flex-1">
        <label className="mb-1 block text-xs font-medium text-stone-600">Details</label>
        <input name="reason" placeholder="Optional" className={`${cls} w-full`} />
      </div>

  ...
  return (
    <>
      ...
      <div className="mb-6 flex items-center gap-4 rounded-2xl border border-slate-200 bg-white p-5">
        ...
        <div>
          <p className="text-lg font-semibold text-slate-800">{basics.label}</p>
          <p className="text-sm text-slate-500">{basics.email ?? "—"}{basics.employeeNo ? ` · ID ${basics.employeeNo}` : ""}</p>
          <div className="mt-1 flex flex-wrap gap-1">
            {basics.roleKeys.map((r) => (
              <span key={r} className="rounded-full bg-slate-100 px-2 py-0.5 text-[11px] capitalize text-slate-600">{roleLabel(r)}</span>
            ))}
          </div>
        </div>
      </div>

      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {accounts.map(({ account, balances }) => (
          <Link
            ...
            className={`rounded-2xl border bg-white p-4 transition hover:shadow-md ${account.is_active ? "border-stone-200" : "border-stone-200 opacity-60"}`}
          >
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="font-semibold text-stone-900">{account.label}</p>
                <p className="text-xs text-stone-500">{account.bank_name ?? "—"} {account.account_no_masked ?? ""}</p>
              </div>
            ...
            <p className="mt-3 text-2xl font-bold tabular-nums text-stone-900">{peso(balances.book)}</p>
            <p className="text-xs text-stone-500">Available (book) balance</p>
            <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-stone-500">
              <span>Cleared: <span className="tabular-nums text-stone-700">{peso(balances.cleared)}</span></span>
        ...
        {accounts.length === 0 && <p className="text-sm text-stone-500">No bank accounts yet.</p>}
      </div>

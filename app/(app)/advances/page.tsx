...
export default async function AdvancesPage() {
  ...
  return (
    <>
      <PageHeader title="Cash Advance" subtitle="Request, approve, release & liquidate cash advances." />
      ...
      <div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white">
        <table className="w-full min-w-[820px] text-left text-sm">
          ...
          <tbody>
            {rows.length === 0 && (
              <tr><td colSpan={canApprove ? 7 : 6} className="px-4 py-8 text-center text-slate-500">No cash advances yet.</td></tr>
            )}
            {rows.map((a) => (
              <tr key={a.id} className="border-b border-slate-100 last:border-0">
                ...
                <td className="px-4 py-2.5">
                  ...
                  {a.decision_note && <span className="block text-[11px] text-slate-400">note: {a.decision_note}</span>}
                </td>
                <td className="px-4 py-2.5 text-slate-500">{a.needed_by ?? "—"}</td>
                <td className="px-4 py-2.5">
                  <span className={`rounded-full px-2 py-0.5 text-[11px] font-medium ${TONE[a.status] ?? "bg-slate-100 text-slate-500"}`}>{label(a.status)}</span>
                </td>
      ...
      <p className="mt-2 text-xs text-slate-400">Open a request to record liquidation once funds are released.</p>
    </>
  );
}

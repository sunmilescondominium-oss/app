      ...
      <div className="mx-auto max-w-2xl rounded-2xl border border-slate-200 bg-white p-6 print:border-0 print:p-0">
        <div className="flex items-start justify-between border-b border-slate-300 pb-4">
          <div>
            ...
            <p className="text-sm text-slate-500">Statement of Account — Monthly Bill</p>
          </div>
          <div className="text-right text-sm text-slate-600">
            <p>Date: <strong>{todayManila()}</strong></p>
            <p className="text-slate-400">Unit {unit.unitNumber} · {unit.businessLine}</p>
          </div>
        </div>

        <div className="mt-4 text-sm">
          <p><span className="text-slate-400">Billed to:</span> <strong>{unit.lease?.tenantLabel ?? "—"}</strong></p>
          {unit.lease?.contact && <p className="text-slate-500">{unit.lease.contact}</p>}
          <p className="text-slate-500">{unit.propertyName}</p>
        </div>
        ...
        <table className="mt-5 w-full text-sm">
          ...
          <tbody>
            {lines.length === 0 && (
              <tr><td colSpan={2} className="py-4 text-center text-slate-500">Nothing outstanding.</td></tr>
            )}
            {lines.map((l, i) => (
              <tr key={i} className="border-b border-slate-100">
                <td className="py-2">
                  {l.label}
                  {l.detail && <span className="ml-1 text-xs text-slate-400">({l.detail})</span>}
                </td>

...
export default async function RentalUnitPage({ params }: { params: Promise<{ unitId: string }> }) {
  ...
  const [airbnbOrders, airbnbRequests] = unit.lease && unit.businessLine === "airbnb"
    ? await Promise.all([listAirbnbOrders(unit.lease.id), listAirbnbRequests(unit.lease.id)])
    : [[], []];

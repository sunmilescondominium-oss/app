  ...
  return (
    <>
      ...
      <div className="mb-6 grid gap-3 sm:grid-cols-3">
        <div className="rounded-2xl border border-stone-200 bg-white p-4"><p className="text-xs text-stone-500">Monthly association dues</p><p className="mt-1 text-lg font-semibold tabular-nums">{peso(unit.monthlyDues)}</p></div>
        <div className="rounded-2xl border border-stone-200 bg-white p-4"><p className="text-xs text-stone-500">Unpaid total</p><p className="mt-1 text-lg font-semibold tabular-nums text-rose-700">{peso(unit.unpaidTotal)}</p></div>
        <div className="rounded-2xl border border-stone-200 bg-white p-4"><p className="text-xs text-stone-500">Area</p><p className="mt-1 text-lg font-semibold tabular-nums">{unit.areaSqm} sqm</p></div>
      </div>
          ...
          <tbody>
            ...
            {dues.map((d) => (
              <tr key={d.id} className="border-b border-stone-100 last:border-0">
                <td className="px-4 py-2.5 capitalize">{d.category.replace("_", " ")}{d.remarks ? <span className="block text-[11px] text-stone-400">{d.remarks}</span> : null}</td>

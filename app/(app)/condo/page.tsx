...
export default async function CondoPage() {
  ...
  return (
    <>
      <PageHeader title="Condo Dues" subtitle="Association dues (area × rate), utilities & billing — common-area fund." />
      ...
      <h2 className="mb-2 mt-6 text-sm font-semibold uppercase tracking-wide text-slate-500">Condo units</h2>
      <div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white">
        <table className="w-full min-w-[720px] text-left text-sm">
          ...
          <tbody>
            {units.length === 0 && <tr><td colSpan={6} className="px-4 py-8 text-center text-slate-500">No condo units. Add condo units in Inventory (business line = condo sales) with an area.</td></tr>}
            {units.map((u) => (
              <tr key={u.unitId} className="border-b border-slate-100 last:border-0">
                <td className="px-4 py-2.5">
                  ...
                  <span className="ml-1 text-xs text-slate-400">{u.propertyName}</span>
                </td>
                ...
                <td className={`px-4 py-2.5 text-right tabular-nums ${u.unpaidTotal > 0 ? "text-rose-700" : "text-slate-400"}`}>{u.unpaidTotal ? peso(u.unpaidTotal) : "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
      {settings.bankAccount && <p className="mt-2 text-xs text-slate-400">Common-area fund account: <strong>{settings.bankAccount}</strong> — deposit condo collections here, separate from operating.</p>}
    </>
  );
}

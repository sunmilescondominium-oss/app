...
import { listBooklets, listFormTypes, custodianOptions, FORM_MANAGER_ROLES } from "@/lib/forms/queries";
...
export default async function FormsPage() {
  ...
  const [booklets, types, custodians] = await Promise.all([listBooklets(), listFormTypes(), canManage ? custodianOptions() : Promise.resolve([])]);
  ...
  return (
    <>
      ...
      {canManage && <div className="mb-4"><RegisterBooklet types={types} custodians={custodians} /></div>}
      ...
      <TableSearch placeholder="Search booklets by no., type, custodian…">
        <div className="table-wrap">
          <table className="w-full min-w-[860px] text-left text-sm">
            ...
            <tbody>
              {booklets.length === 0 && <tr><td colSpan={6} className="px-4 py-8 text-center text-stone-500">No booklets registered yet.</td></tr>}
              {booklets.map((b) => (

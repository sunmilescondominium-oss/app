...
export default async function OwnerPage() {
  ...
  const cards = [
    ...
    { label: "Collected today", value: peso(s.todayTotal), tone: "text-slate-900" },
    { label: "How full the properties are", value: `${s.occupancyPct}%`, sub: `${s.occupied} of ${s.totalUnits} units occupied`, tone: "text-slate-900" },
    { label: "Open issues to watch", value: String(s.openIssues), sub: s.escalated > 0 ? `${s.escalated} escalated` : "none escalated", tone: s.openIssues > 0 ? "text-amber-700" : "text-slate-900" },
  ];

  return (
    <div className="text-[18px] leading-relaxed text-slate-900">
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
        {cards.map((c) => (
          <div key={c.label} className="rounded-2xl border-2 border-slate-200 bg-white p-6">
            <p className="text-slate-600">{c.label}</p>
            <p className={`mt-2 text-4xl font-bold tabular-nums ${c.tone}`}>{c.value}</p>
            {c.sub && <p className="mt-1 text-base text-slate-500">{c.sub}</p>}
          </div>
        ))}
      </div>
      ...
      <div className="mt-8 rounded-2xl border-2 border-amber-200 bg-amber-50 p-6">
        ...
        {s.decisions.length === 0 ? (
          <p className="mt-3 text-slate-700">Nothing needs your decision right now. Everything is on track.</p>
        ) : (

...
import type { FormType, BookletRow, SerialRow, CustodyEntry, SerialStatus } from "@/lib/forms/types";

export type { FormType, BookletRow, SerialRow, CustodyEntry, SerialStatus } from "@/lib/forms/types";
...
export async function listBooklets(): Promise<BookletRow[]> {
  const admin = createAdminClient();
  const [{ data: booklets }, { data: types }, { data: serials }, lbl] = await Promise.all([
...
export async function bookletDetail(id: string): Promise<{ booklet: BookletRow; serials: SerialRow[]; custody: CustodyEntry[] } | null> {
  ...
  if (!b) return null;
  const [{ data: type }, { data: serials }, { data: custody }, lbl] = await Promise.all([
